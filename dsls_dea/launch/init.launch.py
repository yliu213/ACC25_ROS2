# load sls model in gazebo and start px4 sitl
# seems time synchronization is not needed for microXRCEAgent, so omit use_sim_time
# the model loading issue is fixed by enviroment hooks indicated in this website:
# https://answers.gazebosim.org/question/29079/
# need to modify setup's package.xml, Cmakelist.txt, and bashrc
...
def generate_launch_description():
    ...
    # package path
    package_path = get_package_share_directory('dsls_dea')

    # world path, will need to modify gazebo classic world file to add link_attacher plugin
    world_path = os.path.join(HOME, 'PX4-Autopilot/Tools/simulation/gazebo-classic/sitl_gazebo-classic/worlds/empty.world') 

    # model path
    model_path = os.path.join(package_path, 'models/double_slung_load_point_mass/double_slung_load_point_mass.sdf')

    os.makedirs(PX4_RUN_DIR, exist_ok=True)

    return LaunchDescription([
        # Set environment variables for Gazebo and PX4
        SetEnvironmentVariable('GAZEBO_PLUGIN_PATH', HOME + '/PX4-Autopilot/build/px4_sitl_default/build_gazebo-classic'), # px4 v1.15.4
