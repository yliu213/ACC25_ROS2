	...
	DSLS_DEA() : Node("dsls_dea_controller")
	{
		...
		auto timer_callback = [this]() -> void {
            ...
            case ControllerPhase::DSLS_ENABLED:
            {
                ...
                offboard0_control_mode_publisher_->publish(mode_msg);
                offboard1_control_mode_publisher_->publish(mode_msg);
                this-> dea_enabled_ = true; // set manually
                break;
            }
            }
        };
        ...
        param_callback_handle_ = this->add_on_set_parameters_callback(
            ...
            -> rcl_interfaces::msg::SetParametersResult
            {
              ...
              for (auto & param : params) {
                ...
                } else if (param.get_name() == "radium_scalar_"){
                    radium_scalar_ = param.as_double();
                    dsls_dea_ref_[0] = r_1_ * radium_scalar_;
                    dsls_dea_ref_[4] = r_2_ * radium_scalar_;
                    dsls_dea_ref_[8] = r_3_ * radium_scalar_;
                    RCLCPP_INFO(this->get_logger(), "Param changed: radium_scalar_=%f", radium_scalar_);
                } else if (param.get_name() == "freq_scalar_"){
                    freq_scalar_ = param.as_double();
                    dsls_dea_ref_[1] = fr_1_ * freq_scalar_;
                    dsls_dea_ref_[5] = fr_2_ * freq_scalar_;
                    dsls_dea_ref_[9] = fr_3_ * freq_scalar_;
                    RCLCPP_INFO(this->get_logger(), "Param changed: freq_scalar_=%f", freq_scalar_);
                } else if (param.get_name() == "r_1_"){
                    r_1_ = param.as_double();
                    dsls_dea_ref_[0] = r_1_;
                    RCLCPP_INFO(this->get_logger(), "Param changed: r_1_=%f", dsls_dea_ref_[0]);
                } else if (param.get_name() == "r_2_"){
                    r_2_ = param.as_double();
                    dsls_dea_ref_[4] = r_2_;
                    RCLCPP_INFO(this->get_logger(), "Param changed: r_2_=%f", dsls_dea_ref_[4]);
                } else if (param.get_name() == "r_3_"){
                    r_3_ = param.as_double();
                    dsls_dea_ref_[8] = r_3_;
                    RCLCPP_INFO(this->get_logger(), "Param changed: r_3_=%f", dsls_dea_ref_[8]);
                } else if (param.get_name() == "fr_1_"){
                    fr_1_ = param.as_double();
                    dsls_dea_ref_[1] = fr_1_;
                    RCLCPP_INFO(this->get_logger(), "Param changed: fr_1_=%f", dsls_dea_ref_[1]);
                } else if (param.get_name() == "fr_2_"){
                    fr_2_ = param.as_double();
                    dsls_dea_ref_[5] = fr_2_;
                    RCLCPP_INFO(this->get_logger(), "Param changed: fr_2_=%f", dsls_dea_ref_[5]);
                } else if (param.get_name() == "fr_3_"){
                    fr_3_ = param.as_double();
                    dsls_dea_ref_[9] = fr_3_;
                    RCLCPP_INFO(this->get_logger(), "Param changed: fr_3_=%f", dsls_dea_ref_[9]);
                } else if (param.get_name() == "c_1_"){
                    c_1_ = param.as_double();
                    dsls_dea_ref_[2] = c_1_;
                    RCLCPP_INFO(this->get_logger(), "Param changed: c_1_=%f", dsls_dea_ref_[2]);
                } else if (param.get_name() == "c_2_"){
                    c_2_ = param.as_double();
                    dsls_dea_ref_[6] = c_2_;
                    RCLCPP_INFO(this->get_logger(), "Param changed: c_2_=%f", dsls_dea_ref_[6]);
                } else if (param.get_name() == "c_3_"){
                    c_3_ = param.as_double();
                    dsls_dea_ref_[10] = c_3_;
                    RCLCPP_INFO(this->get_logger(), "Param changed: c_3_=%f", dsls_dea_ref_[10]);
                } else {
...
void DSLS_DEA::gazeboCb(const gazebo_msgs::msg::LinkStates::SharedPtr msg){
    ...
    if(dea_enabled_){
        ...
        else applyDSLSDEAController(state18_, dea_xi4_, dea_k_, dea_param_, dsls_dea_ref_, this->get_clock()->now().seconds() - dea_start_time_);
...
int DSLS_DEA::executeMission(void) {
    ...
    if(!mission_start_) {
        saveDSLSDEARef();
        RCLCPP_INFO(this->get_logger(), "[exeMission] Mission Start");
        mission_start_ = true;        
    }
...
int DSLS_DEA::checkMissionStage(double mission_time_span) {
...
int DSLS_DEA::setDEASetpoint(double dea_setpoint[6]) {
    dsls_dea_ref_[0] = 0.0;
    dsls_dea_ref_[4] = 0.0;
    dsls_dea_ref_[8] = 0.0;
...
int DSLS_DEA::setDEATrajectory(double dea_trajectory[15]){
    for(int i = 0; i<15; i++){
...
int DSLS_DEA::saveDSLSDEARef(void) {
    for(int i=0; i<15; i++){
...
int DSLS_DEA::loadDSLSDEARef(void) {
    for(int i=0; i<15; i++){
...
int DSLS_DEA::applyDSLSDEAController(
    DSlsState state18, 
    DEAState &dea_xi4, 
    const double dea_k[24], 
    const double dea_param[4], 
    ...
    // LPF debug msg
    lpf_data_.header.stamp = this->get_clock()->now();
    ...
    // system state msg
    // this have bugs, since the bool are false, so skip this
    //if(time_sync_debug_enabled_) RCLCPP_INFO_STREAM(this->get_logger(), "[applyDSLSDEA] Time:" << this->get_clock()->now());
...
Vector3 DSLS_DEA::applyLPFVector3(
    Vector3 v, Vector3 v_last, Vector3 v_last_2, Vector3 v_dot_last, Vector3 v_dot_last_2, 
    double diff_time, double tau, double xi, double omega, int order
    ){
    Vector3 v_dot;
